...
class UtilsTest extends PHPUnit_Framework_TestCase
{
  ...
  public function testComposeSolrQueryFromAttrsGroup(){
    ...
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&fq=+groups:some_group_name&rows=1000&sort=metadata_modified+desc");
  }

  public function testComposeSolrQueryFromAttrsOrganization(){
    $attrs = array('organization' => 'some_organization_name');
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&fq=+owner_org:some_organization_name&rows=1000&sort=metadata_modified+desc");
  }

  public function testComposeSolrQueryFromAttrsType(){
    $attrs = array('type' => 'some_type');
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&fq=+type:some_type&rows=1000&sort=metadata_modified+desc");
  }

  public function testComposeSolrQueryFromAttrsOneId(){
    $attrs = array('ids' => 'some_id');
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&fq=+id:(some_id)&rows=1000&sort=metadata_modified+desc");
  }

  public function testComposeSolrQueryFromAttrsNoId(){
    $attrs = array('ids' => array());
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&rows=1000&sort=metadata_modified+desc");
  }

  public function testComposeSolrQueryFromAttrsIds(){
    $attrs = array('ids' => array('some_id','other_id'));
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&fq=+id:(some_id OR other_id)&rows=1000&sort=metadata_modified+desc");
  }

  public function testComposeSolrQueryFromAttrsFilterFields(){
    $attrs = array('filter_fields' => '{"spatial-text":"England","date":"2015"}');
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&fq=+spatial-text:England+date:2015&rows=1000&sort=metadata_modified+desc");
  }

  public function testComposeSolrQueryFromAttrsLimit(){
    $attrs = array('limit' => '10');
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&rows=10&sort=metadata_modified+desc");
  }
  ...
  public function testComposeSolrQueryFromAttrsFilter(){
    ...
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&fq=+num_resources:[1 TO *]&rows=1000&sort=metadata_modified+desc");
  }

  public function testComposeSolrQueryFromAttrsLimitAndPage(){
    $attrs = array('limit' => '10', 'page' => '2');
    $arguments = compose_solr_query_from_attrs($attrs);
    $this->assertContains($arguments,"q=%2A%3A%2A&rows=10&start=10&sort=metadata_modified+desc");
  }
